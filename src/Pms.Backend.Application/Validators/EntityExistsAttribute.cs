...
/// <summary>
/// Validation attribute to check if an entity exists in the database
/// </summary>
public class EntityExistsAttribute : ValidationAttribute
{
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        var validationService = serviceProvider.GetService<IAddressValidationService>();
        if (validationService == null)
        {
            return new ValidationResult("Address validation service not available");
        }

        // Validate entity type first
        if (!validationService.ValidateEntityCanHaveAddresses(entityType))
        {
