...
public class MemberValidationService : IMemberValidationService
{
    ...
    private void ValidateContactDataDto(CreateMemberCompleteDto dto, MemberValidationResult result)
    {
        ...
        if (dto.ContactInfo?.Any(c => c.Type == ContactType.Email && !string.IsNullOrWhiteSpace(c.Value)) == true)
        {
            hasEmail = true;
        }

        // Verificar se há telefone nos contatos
        if (dto.ContactInfo?.Any(c => (c.Type == ContactType.Mobile || c.Type == ContactType.Landline || c.Type == ContactType.WhatsApp) && !string.IsNullOrWhiteSpace(c.Value)) == true)
        {
            hasPhone = true;
        }
