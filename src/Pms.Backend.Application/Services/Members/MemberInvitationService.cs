...
public partial class MemberService : IMemberService
{
    ...
    public async Task<BaseResponse<LoginResponseDto>> ActivateMemberAsync(ActivateMemberRequestDto request, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                return BaseResponse<LoginResponseDto>.ErrorResult("Password and confirmation do not match");
            }

            // Validate password strength
            if (request.Password.Length < 8)
            {
                return BaseResponse<LoginResponseDto>.ErrorResult("Password must be at least 8 characters long");
            }
            ...
            if (member == null)
            {
                return BaseResponse<LoginResponseDto>.ErrorResult("Invalid or expired activation token");
            }
            ...
            {
                return BaseResponse<LoginResponseDto>.ErrorResult("User credential not found");
            }
            ...
            if (request.MemberInfo != null)
            {
                if (!string.IsNullOrEmpty(request.MemberInfo.Cpf))
                {
                    ...
                    {
                        return BaseResponse<LoginResponseDto>.ErrorResult("CPF already exists");
                    }
            ...
            return BaseResponse<LoginResponseDto>.SuccessResult(response, "Member activated successfully");
        }
        catch (Exception ex)
        {
            return BaseResponse<LoginResponseDto>.ErrorResult($"Error activating member: {ex.Message}");
        }
    }
