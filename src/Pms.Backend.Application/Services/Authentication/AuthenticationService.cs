...
namespace Pms.Backend.Application.Services.Authentication;
...
public class AuthenticationService : IAuthenticationService
{
    ...
    public async Task<BaseResponse<LoginResponseDto>> LoginAsync(LoginRequestDto request, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!emailContacts.Any())
            {
                ...
                return BaseResponse<LoginResponseDto>.ErrorResult("E-mail ou senha inválidos");
            }
        ...
        catch (Exception ex)
        {
            ...
            return BaseResponse<LoginResponseDto>.ErrorResult("Erro interno do servidor");
        }
    }
    ...
    public async Task<BaseResponse<LoginResponseDto>> RefreshTokenAsync(string token, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                return BaseResponse<LoginResponseDto>.ErrorResult("Token inválido");
            }

            var userInfo = GetUserFromToken(token);
            if (userInfo == null)
            {
                return BaseResponse<LoginResponseDto>.ErrorResult("Não foi possível extrair informações do usuário");
            }
            ...
            if (member == null)
            {
                return BaseResponse<LoginResponseDto>.ErrorResult("Usuário não encontrado");
            }
            ...
            {
                return BaseResponse<LoginResponseDto>.ErrorResult("Usuário inativo");
            }
    ...
    public BaseResponse<object> ValidateTokenWithUserInfo(string token)
    {
        try
        {
            ...
            {
                return BaseResponse<object>.ErrorResult("Token é obrigatório");
            }

            var isValid = ValidateToken(token);
            if (!isValid)
            {
                return BaseResponse<object>.ErrorResult("Token inválido");
            }

            var userInfo = GetUserFromToken(token);
            if (userInfo == null)
            {
                return BaseResponse<object>.ErrorResult("Não foi possível extrair informações do usuário");
            }
        ...
        catch (Exception ex)
        {
            ...
            return BaseResponse<object>.ErrorResult("Erro interno do servidor");
        }
    }
    ...
    public BaseResponse<UserInfoDto> GetUserInfoFromToken(string token)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(token))
            {
                return BaseResponse<UserInfoDto>.ErrorResult("Token é obrigatório");
            }

            if (!ValidateToken(token))
            {
                return BaseResponse<UserInfoDto>.ErrorResult("Token inválido");
            }

            var userInfo = GetUserFromToken(token);
            if (userInfo == null)
            {
                return BaseResponse<UserInfoDto>.ErrorResult("Não foi possível extrair informações do usuário");
            }
        ...
        catch (Exception ex)
        {
            ...
            return BaseResponse<UserInfoDto>.ErrorResult("Erro interno do servidor");
        }
    }
